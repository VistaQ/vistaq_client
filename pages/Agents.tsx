  ...
  useEffect(() => {
    const fetchGroupData = async () => {
      if (currentUser?.role === UserRole.GROUP_LEADER && currentUser.groupId) {
        ...
        const usersCacheKey = buildCacheKey(userId, `group_users_${currentUser.groupId}`);
        const prospectsCacheKey = buildCacheKey(userId, `group_prospects_${currentUser.groupId}`);
        ...
        if (cachedProspects) setGroupProspects(cachedProspects);
        ...
        try {
          if (!cachedUsers) {
            ...
            setGroupUsers(users);
            setCache(usersCacheKey, users, userId);
          }
