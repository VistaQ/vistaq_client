...
const Group: React.FC = () => {
        ...
        {visibleGroups.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {visibleGroups.map(group => {
              ...
              const gProspectsYTD = gProspects.filter(p => isWithinDateRange(p.createdAt, startYTD, endYTD)).length;
  ...
  if (selectedAgentId) {
    ...
    const aMtdProspects = agentProspects.filter(p => isWithinDateRange(p.createdAt, startMTD, endMTD)).length;
    ...
    const aMtdSalesACE = aMtdSalesArr.reduce((sum, p) => sum + ((p.productsSold || []).reduce((s, prod) => s + (prod.aceAmount || 0), 0)), 0);
    ...
    const aProspectsYTD = agentProspects.filter(p => isWithinDateRange(p.createdAt, startYTD, endYTD)).length;
